...
const SuperBundleDetailModal = ({ bundle, onClose, onBuy }) => {
  const getStockDetails = (symbol) => {
    return MOCK_STOCKS.find(s => s.symbol === symbol) || { name: symbol, price: 0, logo: '📊' };
  };

  const totalCost = bundle.stocks.reduce((sum, symbol) => {
      ...
      <div className="modal-content pixel-border" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="bundle-stocks-list">
            ...
            {bundle.stocks.map(symbol => {
              const stock = getStockDetails(symbol);
              return (
                <div key={symbol} className="bundle-stock-item">
                  <span className="bundle-stock-logo">{stock.logo}</span>
                  <div className="bundle-stock-info">
                    <div className="bundle-stock-name">{stock.name}</div>
                    <div className="bundle-stock-symbol">{symbol}</div>
                  </div>
                  <div className="bundle-stock-price">${stock.price.toFixed(2)}</div>
                </div>
              );
            })}
          </div>
          ...
          <div className="bundle-total">
            ...
            <span className="bundle-total-value">${totalCost.toFixed(2)}</span>
          </div>
        </div>
        
        <div className="modal-footer">
          <button className="btn-secondary" onClick={onClose}>CANCEL</button>
          <button className="btn-primary" onClick={() => onBuy(bundle)}>BUY BUNDLE</button>
        </div>
      </div>
    </div>
  );
